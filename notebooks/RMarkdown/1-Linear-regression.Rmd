...
Great, we have some linear algebra working in Python. Now we can efficiently compute the predictions for any number of different sets of $\theta$ and different training examples (still, only one feature in this case, the house size):
...
sig = data.sqft.std() # std
SIZE = ((data.sqft - mu)/sig).values
...
We could visualize how our linear regression evolves after each iteration.

```{python}
print(pd.DataFrame(descentList[0], columns=["theta0", "theta1"]).head())
```
...
<script>
  ...
  var scalingCoef = {{mu: {mu}, sig: {sig}}}
  var gradientSteps = {pd.DataFrame(descentList[0], columns=["theta0", "theta1"]).to_json(orient="records")}
</script>
""")
...
<script>
// unscale the theta coefs (they have been obtained with feature normalization)
gradientSteps = gradientSteps.map(d => {
  var {theta0, theta1} = d
  theta0 = theta0-(theta1*scalingCoef.mu)/scalingCoef.sig
  theta1 = theta1/scalingCoef.sig
...
We could then calculate the $RMSE$ to evaluate the accuracy of our model. Let's define a function for that:
...
print(rmse(trueValues, predictions))
```
...
ax.set_ylabel("True vs Predicted price ($)")
